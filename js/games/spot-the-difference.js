...
import { PHOTO_PAIRS as GENERATED_PAIRS } from '../photo-pairs.js';
...
// ── Photo pairs ──
// The live list comes from js/photo-pairs.js, generated by
// scripts/prepare_manual_edits.py. A pair appears there only when both
// assets/photos/<base>__edited.jpg (hand-edited, differing from the base)
// and a 5-diff assets/photos/<base>__mods.json exist.
...
const ACTIVE_PAIRS = GENERATED_PAIRS.length ? GENERATED_PAIRS : FALLBACK_PAIRS;

// Deterministic progression: level N plays pair (N-1), cycling through every
// hand-edited photo before repeating. Never random.
function pairForLevel(level) {
  const n = Math.max(1, level | 0);
  return ACTIVE_PAIRS[(n - 1) % ACTIVE_PAIRS.length];
}
